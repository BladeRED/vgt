    ...
    AbstractController
{
    ...
    public function displayHomepage()
    {
        ...
        $todayGame =
            $this->gamemanager->findByTodayDate($dateToday,
                $dateLastWeek);
        $todayTime =
            $this->timemanager->findByTodayDate($dateToday,
                $dateLastWeek);
        $todayGamer =
            $this->gamermanager->findByTodayDate($dateToday,
                $dateLastWeek);
    ...
    public function displayGame($id)
    {
        ...
        $timeGame =
            $this->timemanager->findAvgTimeByGameId($id);
        $histTimeGame =
            $this->timemanager->findHistAvgTimeByGameId($id);
        $extraTimeGame =
            $this->timemanager->findExtraAvgTimeByGameId($id);
        $compTimeGame =
            $this->timemanager->findCompAvgTimeByGameId($id);
        ...
        $extraTimeHrs =
            $extraTimeGame["Hours"];
        $extraTimeMins =
            $extraTimeGame["Minuts"];
        $extraTimeSecs =
            $extraTimeGame["Seconds"];
...
//Database check//
                $gamer =
                    $this->gamermanager->getOneByGamerName($_POST["pseudoInput"]);

                if (!is_null($gamer) &&
                    password_verify($_POST["passwordInput"],
                        $gamer->getPassword())) {
                    ...
                    (
                    $errors[] =
                        "Tu t'es trompé de mot de passe et/ou de pseudo!");
                }
                $this->render->display('default/login.twig',
                    ['errors' => $errors]);
            }
    ...
    public function register()
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] ==
            ...
            $errors =
                $this->isValidRegisterForm($errors);
    ...
    public function search()
    {
        ...
        $searchResult =
            trim($_POST["searchResult"]);

        if (strlen($searchResult) >=
            0 &&
            strlen($searchResult) <=
            2) {
            $searches =
    ...
    public function submitInput()
    {
        ...
        $submitResult =
            trim($_POST["submitResult"]);


        if (strlen($submitResult) >=
            0 &&
            strlen($submitResult) <=
            2) {
            $searches =
            ...
            $searches =
                $this->gamemanager->search($submitResult);
     ...
     * @return array
     */
    private function checkLogForm(mixed $password,
                                  array $errors,
                                  mixed $pseudo,
                                  mixed $antibot): array
    {
        ...
        if (!preg_match('/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$ %^&*-]).{8,}$/',
            ...
            $errors[] =
                "Il faut que ton mot de passe ait au moins une majuscule, une minuscule, un chiffre, et un caractère spécial. Ah, et pas d'espace! Oui c'est embêtant, mais c'est pour sécuriser ton compte !";
