...
class game_genreManager extends DBManager {
    ...
    {
        $gamegenre = null;
        $query = $this->bdd->prepare('SELECT * FROM game_genre WHERE Id_Games =:Id_Games');
        $query->execute(["Id_Games" => $id]);
        $result = $query->fetch();

        if ($result) {

            $gamegenre = new game_genre($result["Id_Games"], $result["Id_Genre"]);
    ...
    public function add(game_genre $game_genre)
    {
        ...
        $query = $this->bdd->prepare('INSERT INTO game_genre (Id_Games, Id_Genre) VALUES(:Id_Games, :Id_Genre)');
        $query->execute([
    ...
    public function delete($id)
    {
        ...
        $query = $this->bdd->prepare('DELETE FROM game_genre WHERE Id_Games= :id');
        $query->execute([
            "id" => $id->getIdGame()
        ]);
    }
